...
(function(){
  ...
  function resolveSelectorBundle(bundle){
    ...
    for (var r=0; r<roots.length; r++){
      ...
      if (bundle.xpathAbs && root === document) {
        var xa = byXpath(bundle.xpathAbs); tried.push('xpathAbs:'+bundle.xpathAbs);
        if (xa) return xa;
      }
    }
    // ARIA heuristic
  ...
  function resolveWithMeta(step){
    ...
    for (var b=0;b<bundles.length;b++){
      ...
      if (!candidates.length && bundle.xpathAbs && roots.indexOf(document) !== -1) {
        var xa = byXpath(bundle.xpathAbs); if (xa) candidates.push(xa);
      }
  ...
  function act(step, el){
    console.log('action: ', step.kind, step, el);
    if (step.kind === 'click'){
  ...
  function preWaitForElement(step, timeoutMs, intervalMs){
    var start = Date.now();
    return new Promise(function(resolve, reject){
      var iv = setInterval(function(){
        var r = tryResolve(step);
        if (r && r.el) { clearInterval(iv); resolve(r); return; }
        if (Date.now() - start > timeoutMs) { clearInterval(iv); reject(new Error('pre-wait timeout')); }
      }, intervalMs || 100);
    });
  }
  function tryResolve(step){
    var metaEl = resolveWithMeta(step);
    if (metaEl) return { el: metaEl, via: 'meta', reason: null, tried: 0 };
    return resolveWithAlternatives(step);
  }
  function replay(steps, options){
    options = options || {};
    var stepTimeoutMs = options.stepTimeoutMs || 8000;
    var preWaitEnabled = options.preWaitEnabled !== false; // default true
    var preWaitMs = Math.min(options.preWaitMs || 2000, stepTimeoutMs);
    var i = 0;
    var results = [];
    function next(){
      ...
      var resolvePromise = preWaitEnabled ? preWaitForElement(step, preWaitMs, 100) : Promise.resolve(tryResolve(step));
      return resolvePromise.then(function(resolved){
        var el = resolved && resolved.el;
        if (!el) {
          results.push({ok:false, error:'Element not found', triedSelectors: resolved && resolved.tried});
          return next();
        }
        ...
        // Element not found within pre-wait window
        results.push({ok:false, error:String((err&&err.message)||err), preWait:true});
        console.log('step: ', i, ' err: ', String((err&&err.message)||err));
        return next();
      });
    }
    return next();
  ...
  window.addEventListener('message', (event) => {
    if (event.source === window && event.data.type === 'MCP_REPLAY_TRACE') {
      replay(event.data.trace.steps);
    }
  });
})();